...
if ( !function_exists('remove_plugin_transient') ) {
	function remove_plugin_transient() {
		...
		if ( $file == ('update-core.php' || 'plugins.php') ) {
			...
			if ( isset( $current->last_checked ) && 0 < ( time() - $current->last_checked ) ) {
...
function rach5helper_request($action, $args) {
	global $wp_version;
...
function rach5helper_update_check($checked_data) {
	...
	$request_string = rach5helper_request('basic_check', $request_args);
...
function rach5helper_api_call($def, $action, $args) {
	...
	$request_string = rach5helper_request($action, $args);
